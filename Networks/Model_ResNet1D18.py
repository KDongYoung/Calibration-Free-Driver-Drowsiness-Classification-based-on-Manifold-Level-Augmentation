'''ResNet-1D with 18 layers in PyTorch. + additional augmentation techniques
Reference:
...
from util import MixStyle, mixup_data
...
class Resnet18(nn.Module):
    def __init__(self, args, num_domain, batch_norm=True, batch_norm_alpha=0.1):
        ...
        self.mixstyle_prob=args['mixstyle_prob']
    ...
    def _make_layer(self, block, planes, kernel_size, blocks, stride=1, layer_num=0, dilate=False):
        ...
        if self.aug_type=="mixstyle": 
            if layer_num in self.res_layer:
                layers.append(MixStyle(p=self.mixstyle_prob, alpha=0.1, mix="random_shift", batch_size=self.batch_size, num_domain=self.num_domain))     

        return nn.Sequential(*layers)

    def forward(self, x, target=None, layer_mix=[None]):
        
        if self.aug_type=="manifoldmixup":
            if self.aug_type=="manifoldmixup" and layer_mix == [None]:
                layer_mix = [random.randint(0,5)]
                
            x = x.squeeze(1)
            
            if 0 in layer_mix and self.training:
                x, mix_y = mixup_data(x, target, self.mixup_alpha)

            x = self.conv1(x)
            x = self.layer1(x) # basic block 2개
            
            if 1 in layer_mix and self.training:
                x, mix_y = mixup_data(x, target, self.mixup_alpha)
                
            x = self.layer2(x)
            
            if 2 in layer_mix and self.training:
                x, mix_y = mixup_data(x, target, self.mixup_alpha)
                
            x = self.maxpool(x)
            x = self.layer3(x)
            
            if 3 in layer_mix and self.training:
                x, mix_y = mixup_data(x, target, self.mixup_alpha)
                
            x = self.maxpool(x)
            x = self.layer4(x)
            
            if 4 in layer_mix and self.training:
                x, mix_y = mixup_data(x, target, self.mixup_alpha)
            ...
            if 5 in layer_mix and self.training:
                x, mix_y = mixup_data(x, target, self.mixup_alpha)
